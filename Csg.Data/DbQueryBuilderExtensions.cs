...
{
    /// <summary>
    ///     Creates a <see cref="IDbQueryBuilder" /> associated with the given <see cref="IDbScope" /> which can be used to
    ...
    /// <returns></returns>
    public static IDbQueryBuilder QueryBuilder(this IDbScope scope, string commandText)
    {
        return new DbQueryBuilder(commandText, scope.Connection, scope.Transaction);
    }

    /// <summary>
    ///     Creates a <see cref="IDbQueryBuilder" /> associated with the given <see cref="IDbConnection" /> which can be used
    ///     to build and execute a SQL statement.
    /// </summary>
    /// <param name="connection">The connection with which the resulting <see cref="IDbCommand" /> will be associated with.</param>
    /// <param name="commandText">The table expression, table name, or object name to use as the target of the query.</param>
    /// <returns></returns>
    public static IDbQueryBuilder QueryBuilder(this IDbConnection connection, string commandText)
    {
        return new DbQueryBuilder(commandText, connection);
    }

    /// <summary>
    ///     Creates a <see cref="IDbQueryBuilder" /> associated with the given <see cref="IDbTransaction" /> which can be used
    ///     to build and execute a SQL SELECT statement.
    /// </summary>
    /// <param name="transaction">The transaction with which the resulting <see cref="IDbCommand" /> will be associated with.</param>
    /// <param name="commandText">The table expression, table name, or object name to use as the target of the query.</param>
    /// <returns></returns>
    public static IDbQueryBuilder QueryBuilder(this IDbTransaction transaction, string commandText)
    {
    ...
    /// <summary>
    ///     Executes a T-SQL SELECT statement in format only mode (SET FMTONLY ON) which validates the query and returns the
    ///     result metadata.
    /// </summary>
    /// <param name="query">The query builder instance.</param>
    ...
    /// <returns></returns>
    public static bool GetSchemaTable(this IDbQueryBuilder query, out DataTable schema,
        out ICollection<Exception> errors)
    {
    ...
    /// <returns></returns>
    public static bool GetColumnSchema(this IDbQueryBuilder query, out ReadOnlyCollection<DbColumn> schema,
        out ICollection<Exception> errors)
    {
        var cmd = query.CreateCommand();
        ...
        try
        {
            using (var reader = cmd.ExecuteReader(CommandBehavior.SchemaOnly))
            {
                ...
                {
                    schema = ((IDbColumnSchemaGenerator)reader).GetColumnSchema();
                    return true;
                }
    ...
    {
        return Select(query, fields, false);
    }
    ...
    {
        return Select(query, fields.Select(f => SqlColumn.Parse(query.Root, f)), replace);
    }
    ...
    /// <param name="replace">If true, replaces any existing selections, if false, adds to existing selectoins.</param>
    /// <returns></returns>
    public static IDbQueryBuilder Select(this IDbQueryBuilder query, IEnumerable<ISqlColumn> fields,
        bool replace = false)
    {
        var fork = query.Fork();

        if (replace) query.SelectColumns.Clear();

        foreach (var field in fields) fork.SelectColumns.Add(field);
    ...
    /// <summary>
    ///     Adds a set of WHERE clause conditions by looping over the given collection, and then joining them together with the
    ///     given logic.
    /// </summary>
    /// <param name="query"></param>
    ...
    /// <param name="expression"></param>
    /// <returns></returns>
    public static IDbQueryBuilder WhereAny<TItem>(this IDbQueryBuilder query, IEnumerable<TItem> collection,
        Action<IDbQueryWhereClause, TItem> expression)
    {
        query = query.Fork();
        var group = new DbQueryWhereClause(query.Root, SqlLogic.Or);
    ...
    /// <param name="expression"></param>
    /// <returns></returns>
    public static IDbQueryBuilder WhereAny<TItem>(this IDbQueryBuilder query, IList<TItem> list,
        Action<IDbQueryWhereClause, TItem, int> expression)
    {
        query = query.Fork();
        var group = new DbQueryWhereClause(query.Root, SqlLogic.Or);
    ...
    public static IDbQueryBuilder OrderBy(this IDbQueryBuilder query, params string[] fields)
    {
        ...
        foreach (var field in fields)
            fork.OrderBy.Add(new SqlOrderColumn { ColumnName = field, SortDirection = DbSortDirection.Ascending });
    ...
    public static IDbQueryBuilder OrderByDescending(this IDbQueryBuilder query, params string[] fields)
    {
        var fork = query.Fork();
        foreach (var field in fields)
            fork.OrderBy.Add(new SqlOrderColumn { ColumnName = field, SortDirection = DbSortDirection.Descending });
        return fork;
    }
    ...
    {
        if (timeout <= 0) throw new ArgumentException(ErrorMessage.TimeoutValueMustBeGreater, nameof(timeout));
    ...
    /// <returns></returns>
    public static IDbQueryBuilder AddParameter(this IDbQueryBuilder query, string name, object value, DbType dbType,
        int? size = null)
    {
        var fork = query.Fork();
    ...
    {
        if (query.OrderBy.Count <= 0) throw new InvalidOperationException(ErrorMessage.LimitOrOffsetWithoutOrderBy);
    ...
    /// <summary>
    ///     Sets a SQL statement that will be prefixed to the rendered query with a statement separater afterwards. This can be
    ...
    /// <summary>
    ///     Sets a SQL statment that will be appended to the end of the rendered query after a statement separaeter
    ///     (semicolon).
    /// </summary>
    /// <param name="query"></param>
